...
- **前端跳转（子应用侧）**：登录失效时整页跳 assistant-web 承接的登录页 `/login?returnTo=<encodeURIComponent(pathname+search+hash)>`（assistant-web 自身用同源 base path 拼；其他子应用用其配置的 assistant-web 部署 base，如 developer-portal 的 `REACT_APP_GATEWAY_BASE_URL`）。`returnTo` 只允许站内相对路径，禁止拼完整 URL。各子应用统一使用同款 helper `src/helpers/loginRedirect.ts`（`buildReturnTo`/`markLoginRetry`/`consumeLoginRetry`/`redirectToLoginIfPossible`）。**401 拦截禁止再写 `window.location.href = '/'` 之类丢路径的跳转**，必须走 `redirectToLoginIfPossible()`。
