...
- **前端跳转（子应用侧）**：登录失效时整页跳 assistant-web 承接的登录页 `/login?returnTo=<encodeURIComponent(pathname+search+hash)>`。登录页 base 与 API 同 origin：assistant-web 自身用同源 base path 拼；其他子应用用其前端配置的 API origin（如 developer-portal 的 `VITE_API_ORIGIN`，即后端 `ASSISTANT_BFF_URL` 的前端镜像，空则同源）——因此该 origin 必须同时承载 assistant-web 页面（同域反代保证）。`returnTo` 只允许站内相对路径，禁止拼完整 URL。各子应用统一使用同款 helper `src/helpers/loginRedirect.ts`（`buildReturnTo`/`markLoginRetry`/`consumeLoginRetry`/`redirectToLoginIfPossible`）。**401 拦截禁止再写 `window.location.href = '/'` 之类丢路径的跳转**，必须走 `redirectToLoginIfPossible()`。
- **登录页（assistant-web `/login`）**：`src/views/LoginGateway.tsx` 双接线（router.tsx 已登录路由 + App.tsx 未登录分支按 pathname 特判渲染）。行为：先查 `/api/auth/status`，已登录则本地校验 returnTo 后 `location.replace` 回目标；未登录则自己调 `get-provider` 并跳 `/api/auth/oauth-login/{provider}?returnTo=`（**转发**收到的 returnTo，不重建、不碰 `sso.loginRetry` 标记）。
