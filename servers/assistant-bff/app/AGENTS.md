...
多个同域部署的前端子应用（assistant-web、developer-portal 及后续接入者）共用本服务的 SSO 登录态（同域 HttpOnly Cookie + `/api/auth/*`），“登录后回到登录前页面”统一按以下契约实现，新子应用接入只做两件事，不得各自发明机制。后端侧接入（透传 cookie 调 `/api/auth/userinfo` 识别用户）见 `servers/assistant-bff/docs/bff-auth-integration.md`：
- **前端跳转（子应用侧）**：登录失效时整页跳 assistant-web 承接的登录页 `/login?returnTo=<encodeURIComponent(pathname+search+hash)>`。登录页 base 为 assistant-bff 部署地址：assistant-web 自身用同源 base path 拼；其他子应用用与后端同名的前端变量（developer-portal 为 `ASSISTANT_BFF_URL`，构建时注入，空则回退同源）——该地址需同时承载 assistant-web 页面（同域反代保证）。`returnTo` 只允许站内相对路径，禁止拼完整 URL。各子应用统一使用同款 helper `src/helpers/loginRedirect.ts`（`buildReturnTo`/`markLoginRetry`/`consumeLoginRetry`/`redirectToLoginIfPossible`）。**401 拦截禁止再写 `window.location.href = '/'` 之类丢路径的跳转**，必须走 `redirectToLoginIfPossible()`。
- **登录页（assistant-web `/login`）**：`src/views/LoginGateway.tsx` 双接线（router.tsx 已登录路由 + App.tsx 未登录分支按 pathname 特判渲染）。行为：先查 `/api/auth/status`，已登录则本地校验 returnTo 后 `location.replace` 回目标；returnTo 参数缺失时回退读同源 `document.referrer` 的相对路径（剥掉 origin、忽略登录页自身、过 `isSafeReturnTo` 后才可用；参数存在但非法仍直接回 `/`，不做救援）；未登录则自己调 `get-provider` 并跳 `/api/auth/oauth-login/{provider}?returnTo=`（**转发**收到的 returnTo，不重建、不碰 `sso.loginRetry` 标记）。
- **后端回跳**：登录服务用 `_safe_return_to`（`app/auth/auth_routes.py`）校验 returnTo（以 `/` 开头、非 `//`、无反斜杠、无 scheme/netloc，允许 query/fragment），校验通过后在登录流程结束后 302 回该相对路径；非法或缺失不重定向。仓库内 `oauth-login/{provider}` 为 mock 实现（会用 Referer origin 拼绝对地址以兼容前后端不同端口的本地开发），内网真实 OAuth 实现须在 `oauth_login`（接收并携带 returnTo 穿过授权流程）与 `oauth_callback`（末尾校验回跳）两个方法遵守同一契约，且不要使用 Referer 构造跳转目标。
- **前端无需恢复逻辑**：浏览器被 302 回原 URL 后 SPA 原地重新引导即可。
- **死循环保护**：跳转前写一次性 sessionStorage 标记 `sso.loginRetry`（读即删，由发起跳转的子应用盖章，/login 页面不碰）；重新挂载仍未登录且标记存在 → 展示“无权限”不再跳转；登录成功（`/api/auth/status` ok）即清除标记。
