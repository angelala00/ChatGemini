...
usage_events 表新增 requested_model 列
  后端启动时会自动检测并补齐缺失字段，无需手动迁移，旧版本程序仍可写入

模型选择占比
  忽略旧记录中缺少 requested_model 的数据，避免面板显示“未选择模型”占比
...
pdf文件读取修复
  我现在选择auto的时候，上传的pdf是可以正常读取的，
  选择deepseek的时候，上传pdf，模型就说读不到pdf
  选择qwen的时候，似乎也不对
上传文件类型动态控制
  帮我在gpts里面的gptassistant里面的每个可选模型都加上可上传文件类型，
  auto可上传文档和图片，MODEL_NAME_THINKING和MODEL_NAME_INSTRUCT只能上传文件，不能上传图片，MODEL_NAME_VL只能上传图片，不能上传文档
  然后，对应让这些限制生效，一是在后端校验，上传接口里做校验，二是在前端检验，现在前端检验的逻辑是写死的，现在看来要改动动态的了
