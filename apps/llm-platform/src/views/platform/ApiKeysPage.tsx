...
const ApiKeysPage = ({
    ...
    const createKey = createTokenType === "project" ? `project:${createTokenProjectId ?? ""}` : "user";
    ...
    return (
        <div className="flex flex-col gap-3">
                    ...
                    <div className="overflow-hidden rounded-2xl border border-slate-200">
                        <table className="w-full text-left text-sm">
                            <thead className="bg-slate-50 text-xs uppercase tracking-widest text-slate-400">
                                <tr>
                                    <th className="px-4 py-3 w-28">API Keys</th>
                                    <th className="px-4 py-3 min-w-[120px]">备注</th>
                                    <th className="px-4 py-3 w-24">归属</th>
                            ...
                            <tbody>
                                {apiKeyUser.tokens.map((token) => (
                                    <tr key={token.token} className="border-t border-slate-100 hover:bg-slate-50/30 transition-colors">
                                            ...
                                            {editingToken === token.token ? (
                                                <div className="flex items-center gap-1.5 w-full">
                                                    <input
                                                        ...
                                                        className="flex-1 min-w-0 rounded-lg border border-slate-200 px-2 py-1 text-xs focus:border-blue-400 focus:outline-none focus:ring-1 focus:ring-blue-100"
                                                        placeholder="输入备注..."
                                                        autoFocus
                                                        onKeyDown={(e) => {
                                                ...
                                                <div
                                                    ...
                                                    title="点击编辑备注"
                                                >
                                                    <span className="truncate text-slate-700 block max-w-[calc(100%-16px)]">
                                                        {token.note ? token.note : "点击添加备注"}
                                                    </span>
                                                    <svg xmlns="http://www.w3.org/2000/svg" className="shrink-0 ml-1 h-3 w-3 text-slate-300 opacity-0 transition-opacity group-hover:opacity-100" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        ...
                                        <td className="px-4 py-3 text-slate-600 text-xs whitespace-nowrap">
                                            <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-medium ${
                                                token.enabled ? "bg-emerald-50 text-emerald-700" : "bg-slate-100 text-slate-500"
                                            }`}>
                                                {token.enabled ? "启用" : "禁用"}
                                            </span>
                                        </td>
                                        <td className="px-4 py-3 text-slate-600">
                                            <div className="flex items-center gap-1 whitespace-nowrap">
                                                ...
                                                {token.diagnosticsAuthorized && token.tokenId && (
                                                    <div className="flex items-center gap-1 ml-0.5 border-l border-slate-100 pl-1.5">
                                                        <button
                                                            type="button"
            ...
            <div className="mt-6 flex flex-col gap-4">
                ...
                {isCreating && (
                    <div className="rounded-2xl border border-blue-200 bg-blue-50 p-4">
                        <div className="text-sm font-semibold text-blue-800">
                            创建{createTokenType === "project" ? "项目" : "个人"} Token
                        </div>
                ...
                {!isCreating && ownedProjects.length > 0 ? (
                        ...
                        >
                            <div className="flex flex-wrap items-center justify-between gap-3">
                                ...
                                <button
                                    ...
                                    onClick={() => startCreateToken("project", project.id)}
                                    disabled={
                                        createTokenLoading[`project:${project.id}`] ||
                                        (projectTokenLimit > 0 &&
                                            (projectTokenCounts[project.id] ?? 0) >= projectTokenLimit)
